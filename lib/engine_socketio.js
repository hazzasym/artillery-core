    ...
    for (var key in listeners) {
      if (listeners.hasOwnProperty(key)) {
        ...
        socketio.on(key, function (session) {
          ...
          if (self.pendingCheckFunction) {
            self.pendingCheckFunction();
          }
        });
      }
    }
...
function onCheckConditionDone(context, callback, ee) {
    ...
    delete this.pendingCheckFunction;
...
function checkCondition (context, callback, ee, requestSpec) {
  ...
  if (!self.pendingCheckTimeout && timeout && timeout > 0) {
    self.pendingCheckTimeout = setTimeout(function () {
      self.pendingCheckTimeout = null;
  ...
  if (conditionsMet) {
    ...
    if (!self.pendingCheckFunction) {
      self.pendingCheckFunction = function () {
