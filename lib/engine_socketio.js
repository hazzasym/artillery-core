...
function connectSocket(context, callback, ee, requestSpec) {
  ...
  if (requestSpec.socketlisteners) {
    let listeners = requestSpec.socketlisteners;
    for (var key in listeners) {
      if (listeners.hasOwnProperty(key)) {
        var values = listeners[key];
        self.socketlisteners[key] = {};
        socketio.on(key, function (session) {
          for (var valKey in values) {
            if (values.hasOwnProperty(valKey)) {
              self.socketlisteners[key][valKey] = session[valKey];
            }
          }
          if (context.pendingCheckFunction) {
            context.pendingCheckFunction();
          }
        });
      }
    }
  ...
  for (var key in jsonData) {
    if (jsonData.hasOwnProperty(key)) {
      ...
      if (typeof val === 'string') {
        data[key] = template(context.vars[val] || val, context);
...
function onCheckConditionDone(context, callback, ee) {
    if (context.pendingCheckTimeout) {
      clearTimeout(context.pendingCheckTimeout);
      context.pendingCheckTimeout = null;
    }
    delete context.pendingCheckFunction;
    return callback(null, context);
}

function checkCondition (context, callback, ee, requestSpec) {
  let self = this,
      timeout = requestSpec.timeout,
      conditions = requestSpec.conditions,
      conditionsMet = true;

  if (!context.pendingCheckTimeout && timeout && timeout > 0) {
    context.pendingCheckTimeout = setTimeout(function () {
      context.pendingCheckTimeout = null;
      onCheckConditionDone.apply(self, [context, callback]);
    }, timeout * 1000);
  }

  for (var socketChannel in conditions) {
    if (conditions.hasOwnProperty(socketChannel)) {
      var socketChannelProps = conditions[socketChannel];
      for (var propKey in socketChannelProps) {
        if (socketChannelProps.hasOwnProperty(propKey)) {
          var propDescriptor = socketChannelProps[propKey];
          var currentValue = self.socketlisteners[socketChannel][propKey];
          if (!currentValue) {
            var defaultStatus = propDescriptor["default"];
            currentValue = context.vars[defaultStatus] || defaultStatus;
          }
          if (currentValue !== propDescriptor.match) {
            conditionsMet = false;
            break;
          }
        }
  ...
  if (conditionsMet) {
    ...
    if (!context.pendingCheckFunction) {
      context.pendingCheckFunction = function () {
        checkCondition.apply(self, [context, callback, ee, requestSpec]);
      };
    }
  }
}

SocketIoEngine.prototype.checkStep = function (requestSpec, ee) {
  let self = this;

  let f = function (context, callback) {
    checkCondition.apply(self, [context, callback, ee, requestSpec.check]);
  };
  return f;
};

function getJsonValueFromPath(json, paths) {
...
SocketIoEngine.prototype.step = function (requestSpec, ee) {
  ...
  let f = function(context, callback) {
    ...
    if (requestSpec.check) {
      let delegateFunc = self.checkStep(requestSpec, ee);
      return delegateFunc(context, callback);
    }
    if (!requestSpec.emit) {
    ...
    if (isResponseRequired(requestSpec)) {
      ...
      context.socketio.emit(outgoing.channel, outgoing.data, function (err, data) {
        if (requestSpec.emit.socketcapture) {
          if (err) {
            ee.emit('socket error', err);
            markEndTime(ee, context, startedAt);
            return callback(err, context);
          } else {
            var socketcapture = requestSpec.emit.socketcapture;
            for (var key in socketcapture) {
              if (socketcapture.hasOwnProperty(key)) {
                var alias = socketcapture[key];
                var value = getJsonValueFromPath(data, key);
                context.vars[alias] = value;
              }
            }
            markEndTime(ee, context, startedAt);
            return callback(null, context);
