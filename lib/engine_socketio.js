...
function checkCondition (context, callback, ee, requestSpec) {
  ...
  for (var key in conditions) {
    if (conditions.hasOwnProperty(key)) {
      var conditionSet = conditions[key];
      for (var condition in conditionSet) {
        if (conditionSet.hasOwnProperty(condition)) {
          var val = conditionSet[condition];
          if (self.socketlisteners[key][condition] !== val) {
            conditionsMet = false;
            break;
          }
        }
